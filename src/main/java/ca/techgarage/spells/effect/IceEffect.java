...
public class IceEffect implements Spell {
    ...
    public IceEffect(float damage, MagicShape shape) {
        this.damage = damage;
        this.slowTicks = 60;
        this.shape = shape;
    }
    ...
    public void apply(Level level, LivingEntity caster, LivingEntity target) {
        ...
        if (shape == MagicShape.PROJECTILE) {
            ...
            caster.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, slowTicks, 0));
            caster.addEffect(new MobEffectInstance(MobEffects.RESISTANCE, slowTicks, 1));
